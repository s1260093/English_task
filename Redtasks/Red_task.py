...
def upload():
    ...
    fs.save(fs.filename)
    OutSent = reading_csv(fs.filename)
    # Send info to "uploaded.html"
    return render_template("uploaded.html", Output = OutSent)

#Reading function of CSV
def reading_csv(filename):
    ...
    for i in range(len(value)-1):
        ...
        if value[i] > value[i+1]:
            OutputSentence.append(subject[i] + " " +  downVerbs[x] + " from " + month[i] + " to " + month[i+1] + " by " + str(sub) + ".")
            x+=1
        elif value[i] < value[i+1]:
            OutputSentence.append(subject[i] + " " + upVerbs[y] + " from " + month[i] + " to " + month[i+1] + " by " + str(sub) + ".")
            y+=1
        else:
            OutputSentence.append(subject[i] + " " + sameVerbs[z] + " from " + month[i] + " to " + month[i+1] + ".")
            z+=1
